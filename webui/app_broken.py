...
# 상위 디렉토리에서 HayDay Simulator 모듈 임포트
...
def init_simulator():
    ...
    try:
        ...
        # 로컬라이제이션 초기화 - 상대 경로 사용
        localization_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "hayday_extracted_data", "localization")
...
def api_csv_data(filename):
    ...
    try:
        ...
        # 첫 번째 행이 데이터 타입인 경우 제거 (더 포괄적인 패턴)
        if len(df) > 1:
            first_row = df.iloc[0].astype(str)
            if any(first_row.str.contains('int|String|Boolean|float|string', na=False)):
                df = df.drop(0).reset_index(drop=True)
        
        # 두 번째 행도 데이터 타입인 경우 추가로 제거
        if len(df) > 1:
            first_row = df.iloc[0].astype(str)
            if any(first_row.str.contains('int|String|Boolean|float|string', na=False)):
                df = df.drop(0).reset_index(drop=True)
        
        # 빈 행들 제거 (Name 컬럼이 있는 경우)
        if 'Name' in df.columns:
            # Name이 비어있지 않고 의미있는 데이터가 있는 행만 유지
            df = df[df['Name'].notna() & (df['Name'].astype(str).str.strip() != '') & (df['Name'].astype(str).str.strip() != 'nan')]
            df = df.reset_index(drop=True)
        
        # 로컬라이제이션 적용 (한국어 번역)
        if localization and 'TID' in df.columns:
            # 한국어 번역된 이름 컬럼 추가
            df['Name_KR'] = df['TID'].apply(lambda x: localization.get_text(x, 'kr') if pd.notna(x) else '')
            df['Name_EN'] = df['TID'].apply(lambda x: localization.get_text(x, 'en') if pd.notna(x) else '')
            
            # 번역이 성공한 경우 Name 컬럼을 한국어로 교체
            for idx, row in df.iterrows():
                if pd.notna(row.get('TID')) and row['Name_KR'] and row['Name_KR'] != row['TID']:
                    df.at[idx, 'Name'] = row['Name_KR']
                elif pd.notna(row.get('Name')) and row['Name']:
                    # 원본 Name 유지
                    pass
                else:
                    # TID를 Name으로 사용
                    if pd.notna(row.get('TID')):
                        df.at[idx, 'Name'] = row['TID']
